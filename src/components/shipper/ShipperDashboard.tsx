...
export default function ShipperDashboard({ user, data }: { user: PublicUser; data: ShipperDashboardData }) {
  ...
  useEffect(() => { const supabase = createClient(); const channel = supabase.channel(`shipper-notifications-${user.id}`).on("postgres_changes", {
    event: "INSERT", schema: "public", table: "notifications", filter: `receiver_id=eq.${user.id}`,
  }, () => router.refresh()).subscribe(); return () => { void supabase.removeChannel(channel); }; }, [router, user.id]);
  ...
  return <main className="shipper-page">
    ...
    {dashboardReady && data.profile ? <>
      ...
      {data.activeDeliveries.map((delivery) => <ActiveDeliveryCard key={delivery.orderId} delivery={delivery} profileId={data.profile!.id} pending={pending} run={run} onNotice={setNotice} onRefresh={() => router.refresh()} />)}
