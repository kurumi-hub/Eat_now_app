    ...
    {dashboardReady && data.profile ? <div className="shipper-workspace">
      <nav className="shipper-tabs" aria-label="Điều hướng kênh tài xế">
        ...
        <div className="shipper-tabs__items">{([
          ...
          ["deliveries", `Chuyến giao${data.activeDeliveries.length ? ` (${data.activeDeliveries.length})` : ""}`],
      ...
      <section className="shipper-content">
        ...
        {tab === "deliveries" && <>
          <section className="shipper-map-stage"><div className="shipper-map-stage__heading"><div><p>Điều hướng trực tiếp</p><h2>{currentDelivery ? `${currentDelivery.code} · ${DELIVERY_STATUS[currentDelivery.deliveryStatus] || currentDelivery.deliveryStatus}` : "Chưa có chuyến đang giao"}</h2><span>{currentDelivery ? "Bản đồ hiển thị vị trí tài xế, nhà hàng, điểm giao và tuyến đường hiện tại." : "Bật online và nhận một chuyến để bắt đầu điều hướng."}</span></div>{!currentDelivery && <button type="button" onClick={() => setTab("available")}>Tìm chuyến</button>}</div><div className="shipper-map-stage__map"><LiveOrderMap shipper={liveLocation} restaurant={currentDelivery?.restaurant.lat != null && currentDelivery.restaurant.lon != null ? { lat: currentDelivery.restaurant.lat, lon: currentDelivery.restaurant.lon } : null} destination={currentDelivery?.customer.lat != null && currentDelivery.customer.lon != null ? { lat: currentDelivery.customer.lat, lon: currentDelivery.customer.lon } : null} focus={mapFocus} showRoute={Boolean(currentDelivery)} /></div>{currentDelivery && <LiveTracking onNotice={setNotice} onLocation={setLiveLocation} />}</section>
