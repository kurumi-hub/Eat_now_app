...
import { useCallback, useEffect, useRef, useState, useTransition, type CSSProperties, type FormEvent, type PointerEvent as ReactPointerEvent, type ReactNode } from "react";
...
type ShipperTab = "available" | "deliveries" | "profile";
const SHIPPER_TAB_ICONS: Record<ShipperTab, ReactNode> = {
  available: <SearchOutlinedIcon fontSize="small" />,
...
export default function ShipperDashboard({ user, data: initialData }: { user: PublicUser; data: ShipperDashboardData }) {
  ...
  const [trackingSentAt, setTrackingSentAt] = useState<string>();
  ...
  useEffect(() => { if (window.localStorage.getItem("eatnow-shipper-tracking") === "off") setTrackingEnabled(false); }, []);
  useEffect(() => { const open = dashboardReady && tab === "deliveries"; document.body.classList.toggle("shipper-map-open", open); return () => document.body.classList.remove("shipper-map-open"); }, [dashboardReady, tab]);
  ...
  const toggleTracking = () => setTrackingEnabled((value) => { const next = !value; window.localStorage.setItem("eatnow-shipper-tracking", next ? "on" : "off"); return next; });
  ...
  return <main className={`shipper-page ${tab === "deliveries" && dashboardReady ? "is-delivery-tab" : ""}`}>
    ...
    {dashboardReady && <LiveTracking enabled={trackingEnabled} active={Boolean(currentDelivery)} onNotice={setNotice} onLocation={setLiveLocation} onSentAt={setTrackingSentAt} />}
    ...
    {dashboardReady && data.profile ? <div className="shipper-workspace">
      <nav className="shipper-tabs" aria-label="Điều hướng kênh tài xế">
        ...
        <div className="shipper-tabs__items">{([
          ["available", `Nhận chuyến${availableTripCount ? ` (${availableTripCount})` : ""}`],
          ["deliveries", `Đang giao${data.activeDeliveries.length ? ` (${data.activeDeliveries.length})` : ""}`],
      ...
      <section className="shipper-content">
        ...
        {tab === "deliveries" && <section className="shipper-delivery-screen">
          <section className="shipper-map-stage">{!currentDelivery && <div className="shipper-map-stage__heading"><div><p>Điều hướng trực tiếp</p><h2>Chưa có chuyến đang giao</h2><span>Bật online và nhận một chuyến để bắt đầu điều hướng.</span></div><button type="button" onClick={() => setTab("available")}>Tìm chuyến</button></div>}<div className="shipper-map-stage__map"><LiveOrderMap shipper={liveLocation} restaurant={currentDelivery?.restaurant.lat != null && currentDelivery.restaurant.lon != null ? { lat: currentDelivery.restaurant.lat, lon: currentDelivery.restaurant.lon } : null} destination={currentDelivery?.customer.lat != null && currentDelivery.customer.lon != null ? { lat: currentDelivery.customer.lat, lon: currentDelivery.customer.lon } : null} focus={mapFocus} showRoute={Boolean(currentDelivery)} /></div></section>
          {currentDelivery && <DeliveryBottomSheet delivery={currentDelivery}>{data.route.length > 0 && <RoutePlan stops={data.route} count={data.activeDeliveries.length} />}{data.activeDeliveries.map((delivery) => <ActiveDeliveryCard key={delivery.orderId} delivery={delivery} pending={pending} run={run} onNotice={setNotice} onRefresh={() => void syncDashboard()} />)}</DeliveryBottomSheet>}
        </section>}
        {tab === "profile" && <><section className="shipper-panel shipper-profile-panel"><div className="shipper-panel__heading"><div><p>Thông tin vận hành</p><h2>Hồ sơ tài xế</h2><span>Thông tin đã được EatNow xác minh để nhận chuyến.</span></div><span className="shipper-status is-approved">Đã xác minh</span></div><div className="shipper-profile-grid"><article><span>Họ và tên</span><strong>{data.profile.fullName}</strong></article><article><span>Số điện thoại</span><strong>{application?.phone || user.phone || "Chưa cập nhật"}</strong></article><article><span>Phương tiện</span><strong>{data.profile.vehicleType}</strong></article><article><span>Biển số xe</span><strong>{data.profile.plateNumber}</strong></article><article><span>Trạng thái tài khoản</span><strong>{data.profile.isActive ? "Đang hoạt động" : "Tạm khóa"}</strong></article><article><span>Vị trí cập nhật</span><strong>{data.profile.lastLocationAt ? date(data.profile.lastLocationAt) : "Chưa cập nhật"}</strong></article></div><div className="shipper-profile-actions"><button type="button" className="shipper-profile-location" disabled={pending} onClick={updateLocation}>Cập nhật vị trí hiện tại</button></div></section><section className={`shipper-panel shipper-tracking-setting ${trackingEnabled ? "is-enabled" : ""}`}><div><span>Chia sẻ vị trí</span><strong>{trackingEnabled ? currentDelivery ? "Tracking đang hoạt động" : "Sẵn sàng tự động tracking" : "Tracking đang tắt"}</strong><small>{trackingEnabled ? `Tự động chia sẻ khi có chuyến${trackingSentAt ? ` · cập nhật ${date(trackingSentAt)}` : ""}.` : "Khách sẽ không nhận được vị trí trực tiếp."}</small></div><button type="button" aria-pressed={trackingEnabled} onClick={toggleTracking}>{trackingEnabled ? "Tắt tracking" : "Bật tracking"}</button></section><ShipperWalletPlaceholder /><ShipperHistory items={data.history} /></>}
      </section>
    </div> : null}
  </main>;
}
...
function ShipperHistory({ items }: { items: ShipperDashboardData["history"] }) {
  const pageSize = 10;
  const [page, setPage] = useState(1);
  const pageCount = Math.max(1, Math.ceil(items.length / pageSize));
  useEffect(() => setPage((current) => Math.min(current, pageCount)), [pageCount]);
  const visibleItems = items.slice((page - 1) * pageSize, page * pageSize);
  return <section className="shipper-panel"><div className="shipper-panel__heading"><div><p>{items.length} chuyến gần nhất</p><h2>Lịch sử giao hàng</h2><span>Hiển thị tối đa 10 chuyến trên mỗi trang.</span></div></div><div className="shipper-history">{visibleItems.length ? visibleItems.map((item) => <article key={item.orderId}><div><strong>{item.code}</strong><span>{item.restaurantName}</span></div><div><b>{money(item.earning)}</b><span>{DELIVERY_STATUS[item.deliveryStatus] || item.deliveryStatus} · {date(item.completedAt)}</span></div></article>) : <div className="shipper-empty">Chưa có chuyến đã hoàn thành.</div>}</div>{pageCount > 1 && <nav className="shipper-history-pagination" aria-label="Phân trang lịch sử giao hàng"><button type="button" disabled={page === 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>Trước</button><div>{Array.from({ length: pageCount }, (_, index) => index + 1).map((value) => <button type="button" key={value} className={page === value ? "is-active" : ""} aria-current={page === value ? "page" : undefined} onClick={() => setPage(value)}>{value}</button>)}</div><button type="button" disabled={page === pageCount} onClick={() => setPage((current) => Math.min(pageCount, current + 1))}>Sau</button></nav>}</section>;
}

function LiveTracking({ enabled, active, onNotice, onLocation, onSentAt }: { enabled: boolean; active: boolean; onNotice: (notice: ShipperActionResult) => void; onLocation: (location: LiveMapLocation) => void; onSentAt: (value: string) => void }) {
  const lastSent = useRef(0);
  useEffect(() => {
    if (!enabled || !active) return;
    if (!navigator.geolocation) {
      onNotice({ ok: false, message: "Trình duyệt không hỗ trợ định vị." });
      return;
    }
    const supabase = createClient();
    let watch: number | undefined;
    watch = navigator.geolocation.watchPosition(async (position) => {
      onLocation({ lat: position.coords.latitude, lon: position.coords.longitude, heading: position.coords.heading ?? undefined });
      if (Date.now() - lastSent.current < 8000) return;
      lastSent.current = Date.now();
      const { error } = await supabase.rpc("api_track_shipper_location", {
        ...
        p_device_at: new Date(position.timestamp).toISOString(),
      });
      if (error) {
        if (watch != null) navigator.geolocation.clearWatch(watch);
        onNotice({ ok: false, message: error.message || "Không thể chia sẻ vị trí." });
      } else onSentAt(new Date().toISOString());
    }, () => onNotice({ ok: false, message: "Không thể theo dõi vị trí. Hãy cấp quyền định vị." }), {
      enableHighAccuracy: true,
      maximumAge: 5000,
      timeout: 15000,
    });
    return () => { if (watch != null) navigator.geolocation.clearWatch(watch); };
  }, [active, enabled, onLocation, onNotice, onSentAt]);
  return null;
}

function DeliveryBottomSheet({ delivery, children }: { delivery: ActiveDelivery; children: ReactNode }) {
  const [expanded, setExpanded] = useState(false); const [dragOffset, setDragOffset] = useState(0); const startY = useRef<number | null>(null);
  const pointerDown = (event: ReactPointerEvent<HTMLButtonElement>) => { startY.current = event.clientY; event.currentTarget.setPointerCapture(event.pointerId); };
  const pointerMove = (event: ReactPointerEvent<HTMLButtonElement>) => { if (startY.current == null) return; const delta = event.clientY - startY.current; setDragOffset(Math.max(expanded ? -40 : -520, Math.min(expanded ? 520 : 40, delta))); };
  const finishDrag = (event: ReactPointerEvent<HTMLButtonElement>) => { if (startY.current == null) return; const delta = event.clientY - startY.current; if (delta < -45) setExpanded(true); else if (delta > 45) setExpanded(false); else if (Math.abs(delta) < 8) setExpanded((value) => !value); startY.current = null; setDragOffset(0); };
  return <section className={`shipper-delivery-sheet ${expanded ? "is-expanded" : ""}`} style={{ "--sheet-drag": `${dragOffset}px` } as CSSProperties}><button type="button" className="shipper-delivery-sheet__handle" aria-expanded={expanded} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={finishDrag} onPointerCancel={() => { startY.current = null; setDragOffset(0); }}><i /><div><small>Đơn đang thực hiện</small><strong>{delivery.code} · {DELIVERY_STATUS[delivery.deliveryStatus] || delivery.deliveryStatus}</strong></div><span>{expanded ? "Thu gọn" : "Kéo lên xem chi tiết"}</span></button><div className="shipper-delivery-sheet__body">{children}</div></section>;
}

function SuggestedOfferList({ data, pending, run }: { data: ShipperDashboardData; pending: boolean; run: (task: () => Promise<ShipperActionResult>) => void }) {
  return <section className="shipper-panel shipper-offers"><div className="shipper-panel__heading"><div><p>Ưu tiên cho bạn</p><h2>Chuyến vừa được đề xuất</h2><span>Phản hồi trước khi đề xuất hết thời hạn.</span></div></div><div className="shipper-job-list">{data.offers.map((item) => <article key={item.offerId}><div className="shipper-job__route"><span>Nhận tại</span><strong>{item.restaurantName}</strong><p>{item.restaurantAddress}</p><span>Giao đến</span><strong>{item.deliveryArea}</strong></div><dl><div><dt>Đến quán</dt><dd>{item.pickupDistanceKm == null ? "—" : `${item.pickupDistanceKm.toFixed(1)} km`}</dd></div><div><dt>Quãng giao</dt><dd>{item.deliveryDistanceKm == null ? "—" : `${item.deliveryDistanceKm.toFixed(1)} km`}</dd></div><div><dt>Hết hạn</dt><dd>{date(item.expiresAt)}</dd></div></dl><div className="shipper-job__actions"><strong>{money(item.earning)}</strong><button className="is-secondary" disabled={pending} onClick={() => run(() => rejectDeliveryOfferAction(item.offerId))}>Bỏ qua</button><button disabled={pending} onClick={() => run(() => acceptDeliveryOfferAction(item.offerId))}>Nhận chuyến</button></div></article>)}</div></section>;
}
...
function AvailableList({ data, pending, run, onRefresh }: { data: ShipperDashboardData; pending: boolean; run: (task: () => Promise<ShipperActionResult>) => void; onRefresh: () => void }) {
  const available = data.activeDeliveries.length ? [] : data.available.filter((item) => !item.canBatch);
  return <section className="shipper-panel"><div className="shipper-panel__heading"><div><p>Đơn giao mới</p><h2>Chuyến đang chờ tài xế</h2><span>Mỗi tài xế chỉ nhận một chuyến tại một thời điểm.</span></div><button disabled={pending} onClick={onRefresh}>Làm mới</button></div>{data.activeDeliveries.length ? <div className="shipper-empty"><strong>Bạn đang có một chuyến</strong><span>Hoàn thành hoặc trả chuyến hiện tại trước khi nhận đơn mới.</span></div> : !data.profile?.isOnline ? <div className="shipper-empty"><strong>Bạn đang offline</strong><span>Bật trạng thái online để xem và nhận chuyến.</span></div> : <div className="shipper-job-list">{available.length ? available.map((item) => <article key={item.orderId}><div className="shipper-job__route"><span>Nhận tại</span><strong>{item.restaurantName}</strong><p>{item.restaurantAddress}</p><span>Giao đến</span><strong>{item.deliveryArea}</strong></div><dl><div><dt>Đến quán</dt><dd>{item.pickupDistanceKm == null ? "Chưa xác định" : `${item.pickupDistanceKm.toFixed(1)} km`}</dd></div><div><dt>Quãng giao</dt><dd>{item.deliveryDistanceKm == null ? "—" : `${item.deliveryDistanceKm.toFixed(1)} km`}</dd></div><div><dt>Thu nhập dự kiến</dt><dd>{money(item.earning)}</dd></div></dl><div className="shipper-job__actions"><small>{item.code} · {date(item.createdAt)}</small><button disabled={pending} onClick={() => run(() => acceptDeliveryAction(item.orderId))}>Nhận chuyến</button></div></article>) : <div className="shipper-empty"><strong>Chưa có chuyến phù hợp</strong><span>Hãy cập nhật vị trí hoặc làm mới sau ít phút.</span></div>}</div>}</section>;
}
