...
export default function OwnerOrderConsole({ restaurantId, data: initialData, menu, canReject }: { restaurantId: string; data: OwnerOrderList; menu: OwnerMenuData; canReject: boolean }) {
  ...
  return <section className="owner-orders">
    ...
    {items.length && selectedOrder ? <div className="owner-orders__split">
      <div className="owner-order-list-column"><nav className="owner-order-pagination" aria-label="Phân trang đơn hàng"><p>Hiển thị {pageStart}–{pageEnd} / {data.total}</p><div><button type="button" aria-label="Trang trước" disabled={page === 1 || loadingPage} onClick={() => { setPage((value) => Math.max(1, value - 1)); setSelectedOrderId(""); }}>‹</button>{paginationItems.map((item) => typeof item === "number" ? <button type="button" key={item} className={page === item ? "is-active" : ""} aria-current={page === item ? "page" : undefined} disabled={loadingPage} onClick={() => { setPage(item); setSelectedOrderId(""); }}>{item}</button> : <span key={item}>…</span>)}<button type="button" aria-label="Trang sau" disabled={page === totalPages || loadingPage} onClick={() => { setPage((value) => Math.min(totalPages, value + 1)); setSelectedOrderId(""); }}>›</button></div></nav><div className={`owner-order-list owner-order-list--compact${loadingPage ? " is-loading" : ""}`}>{items.map((item) => {
