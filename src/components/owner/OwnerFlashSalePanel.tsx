    ...
    <div className="owner-flash__grid">
      <form className="owner-card owner-flash__form" onSubmit={submit}>
        ...
        {data.campaigns.length === 0 ? <p>Hiện chưa có chiến dịch nhận đề xuất.</p> : <>
          ...
          <div><label>Giá sale<input name="salePrice" type="number" min="1" max={food ? food.basePrice - 1 : undefined} step="1000" required /></label><label>Tổng số suất<input name="stockLimit" type="number" min="1" max="100000" required /></label></div>
      ...
      <section className="owner-card owner-flash__proposals">
        ...
        {data.proposals.length === 0 ? <p>Chưa có đề xuất nào.</p> : data.proposals.map((proposal) => <article key={proposal.id}>
          <div><strong>{proposal.foodName}</strong><span className={`is-${proposal.status}`}>{STATUS[proposal.status]}</span></div>
          ...
          {proposal.status === "pending" && <button type="button" disabled={pending} onClick={() => run(() => cancelFlashSaleProposalAction(proposal.id))}>Hủy đề xuất</button>}
        </article>)}
      </section>
    </div>
  </div>;
