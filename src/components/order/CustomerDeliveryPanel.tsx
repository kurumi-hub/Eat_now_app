...
export default function CustomerDeliveryPanel({ initial }: { initial: CustomerDeliveryData }) {
  ...
  return <section className="customer-delivery-card">
    <div className="customer-delivery-heading"><div><span>Theo dõi giao hàng</span><h2>{["proof_submitted", "awaiting_customer_confirmation"].includes(initial.deliveryStatus) ? "Tài xế đang chờ bạn xác nhận" : ["delivery_review", "disputed"].includes(initial.deliveryStatus) ? "Đang xử lý phản hồi" : "Vị trí tài xế"}</h2></div><i className={location ? "is-live" : ""}>{location ? "Trực tiếp" : "Chưa có tín hiệu"}</i></div>
    ...
    {initial.proof && <div className="customer-proof"><div><strong>Ảnh giao hàng</strong><span>Gửi lúc {time(initial.proof.submittedAt)}</span></div>{initial.proofUrl ? <img src={initial.proofUrl} alt="Bằng chứng tài xế đã giao đơn" /> : <p>Không thể mở ảnh lúc này. Hãy tải lại trang.</p>}{initial.proof.note && <blockquote>{initial.proof.note}</blockquote>}</div>}
